...
public:
    ...
    void push_back(const T& value) {    //원소 value를 data의 맨 뒤(size()기준)에 추가함
        if(currentCapacity > currentSize){
            *(data + currentSize + 1) = value;  //주소(맨 뒤 빈 곳)의 값을 value로 지정
            currentSize++;
        }
        else {
            //크기를 추가한다
    ...
    void sortData() {
        ...
        std::sort(data, data + currentCapacity);
        //
    }
...
int main()
{
    ...
    std::cout << "pop_back 실행 후 stringVector size : " << stringVector.size() << "  "<<std::endl;
